...
class TestParseUri:
    ...
    def test_parses_action_from_netloc() -> None:
        ...
        result = parse_uri('spurtle://install?manifest=https://example.com')
        assert result['action'] == 'install'

    @staticmethod
    def test_parses_query_parameters() -> None:
        """Verify query parameters are included in the result."""
        result = parse_uri('spurtle://install?manifest=https://example.com/foo.json')
    ...
    def test_parses_multiple_query_values() -> None:
        ...
        result = parse_uri('spurtle://install?manifest=https://a.com/a.json&manifest=https://b.com/b.json')
    ...
    def test_action_only_uri() -> None:
        ...
        result = parse_uri('spurtle://update')
        assert result == {'action': 'update'}

    @staticmethod
    def test_unknown_action() -> None:
        """Verify unknown actions are returned as-is."""
        result = parse_uri('spurtle://unknown?foo=bar')
        assert result['action'] == 'unknown'
        assert result['foo'] == ['bar']

    @staticmethod
    def test_legacy_scheme_still_parses() -> None:
        """Verify legacy synodic:// URIs continue to parse."""
