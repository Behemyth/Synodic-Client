...
class TestSyncFromConfig:
    ...
    def test_channel_dev() -> None:
        ...
        window = _make_window(make_resolved_config(update_channel='dev'))
        window.sync_from_config()
    ...
    def test_update_source_set() -> None:
        ...
        window = _make_window(make_resolved_config(update_source='https://example.com'))
        window.sync_from_config()
    ...
    def test_auto_update_interval_custom() -> None:
        ...
        custom_interval = 60
        window = _make_window(make_resolved_config(auto_update_interval_minutes=custom_interval))
        window.sync_from_config()
    ...
    def test_tool_update_interval_custom() -> None:
        ...
        custom_interval = 45
        window = _make_window(make_resolved_config(tool_update_interval_minutes=custom_interval))
        window.sync_from_config()
...
class TestSettingsCallbacks:
    ...
    def test_channel_change_to_stable() -> None:
        ...
        config = make_resolved_config(update_channel='dev')
        window = _make_window(config)
        window.sync_from_config()

        new_config = make_resolved_config(update_channel='stable')
        with patch.object(window._store, 'update', return_value=new_config) as mock_update:
    ...
    def test_source_change() -> None:
        ...
        new_config = make_resolved_config(update_source='https://custom.example.com')
        with patch.object(window._store, 'update', return_value=new_config) as mock_update:
    ...
    def test_source_blank_sets_none() -> None:
        ...
        config = make_resolved_config(update_source='https://old.example.com')
        window = _make_window(config)

        new_config = make_resolved_config(update_source=None)
        with patch.object(window._store, 'update', return_value=new_config) as mock_update:
    ...
    def test_auto_start_removes_startup_when_frozen() -> None:
        ...
        config = make_resolved_config(auto_start=True)
        window = _make_window(config)
...
class TestSyncDoesNotEmit:
    ...
    def test_sync_no_signal() -> None:
        ...
        config = make_resolved_config(update_channel='dev', auto_update_interval_minutes=60)
        window = _make_window(config)
...
class TestStoreConfig:
    ...
    def test_sync_after_store_update_uses_new_timestamp() -> None:
        ...
        window = _make_window(make_resolved_config(last_client_update=None))
        assert not window._last_client_update_label.text()

        new_config = make_resolved_config(last_client_update='2026-03-09T12:00:00+00:00')
